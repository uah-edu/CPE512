...
void compute_temp() {
    ...
    for (int i=0;i<num_iterations;i++) {
        ...
        if (rank < numprocs - 1) {
            MPI_Sendrecv(&temp[active_rows_on_proc * total_cols], total_cols, MPI_DOUBLE, up_pr, 123,
                &temp[(active_rows_on_proc + 1) * total_cols], total_cols, MPI_DOUBLE, up_pr, MPI_ANY_TAG,
                MPI_COMM_WORLD, &status);
        }

        // Send data to the down neighbor and receive from it
        if (rank > 0) {
            MPI_Sendrecv(&temp[total_cols], total_cols, MPI_DOUBLE, down_pr, 123, &temp[0], total_cols, MPI_DOUBLE,
