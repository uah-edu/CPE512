...
/*
    To compile on the DMC
        GNU Compiler
            module load openmpi
            mpicc heat_2d_loc_blk_MPI.c -o heat_2d_loc_blk_MPI_gnu -O3
 
   To executed on the DMC
      GNU Compiler
...
int main (int argc, char *argv[]){
    ...
    if (argc!=3 && argc!=4) {
        if (rank==0) {
            printf("Usage: mpirun -np [No. procs] %s",argv[0]);
            printf(" [Dim n] [No. Iterations] [Suppress Output]\n");
            fflush(stdout);
        }
        MPI_Finalize();
        exit(0);
