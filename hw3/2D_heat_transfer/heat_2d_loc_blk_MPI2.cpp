...
To compile on the DMC
   GNU Compiler
      module load mpich
      mpic++ heat_2d_loc_blk_MPI2.cpp -o heat_2d_loc_blk_MPI2_gnu -std=c++11 -O3

To execute on the DMC
   GNU Compiler
      run_script_mpi heat_2d_loc_blk_MPI2_gnu.sh
    ...
    for (int i=0;i<num_iterations;i++) {
        if (rank%2==0) { // even numbered processes
            MPI::COMM_WORLD.Send(&temp[active_rows_on_proc*total_cols],
                    total_cols, MPI::DOUBLE,up_pr,123);
            MPI::COMM_WORLD.Recv(&temp[(active_rows_on_proc+1)*total_cols],
                    total_cols,MPI::DOUBLE,up_pr,MPI_ANY_TAG,status);
            if (rank>0) {
                MPI::COMM_WORLD.Send(&temp[total_cols],total_cols,MPI::DOUBLE,
                        down_pr,123);
                MPI::COMM_WORLD.Recv(&temp[0],total_cols,MPI::DOUBLE,down_pr,
                        MPI_ANY_TAG,status);
            }
        }
...
void print_temp(void) {
    ...
    if (rank!=0) {
        ...
        MPI::COMM_WORLD.Recv(&flg,1,MPI::CHAR,down_pr,MPI_ANY_TAG,
           status);
    }
    else {
    ...
    if (rank!=numprocs-1) {
        MPI::COMM_WORLD.Send(&flg,1,MPI::CHAR,up_pr,123);
...
unsigned long long int checksum(void) {
    ...
    if (rank!=0) {
        // if not rank 0 wait until adjacent left process has completed
        MPI::COMM_WORLD.Recv(&sum,sizeof(double),MPI_CHAR,down_pr,MPI_ANY_TAG,
                status);
    }
    else {
    ...
    if (rank!=numprocs-1) {
        MPI::COMM_WORLD.Send(&sum,sizeof(double),MPI_CHAR,up_pr,123);
        if (rank==0) {
