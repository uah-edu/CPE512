...
void compute_temp() {
    ...
    for (int i=0;i<num_iterations;i++) {
        // Initialize non-blocking sends
        if (rank < numprocs - 1) {
            MPI_Isend(&temp[active_rows_on_proc * total_cols], total_cols,
                    MPI_DOUBLE, up_pr, 123, MPI_COMM_WORLD, &send_req[0]);
        }
        if (rank > 0) {
            MPI_Isend(&temp[total_cols], total_cols, MPI_DOUBLE, down_pr, 123,
                    MPI_COMM_WORLD, &send_req[1]);
        }

        // Initialize non-blocking receives
        if (rank < numprocs - 1) {
            MPI_Irecv(&temp[(active_rows_on_proc + 1) * total_cols], total_cols,
                    MPI_DOUBLE, up_pr, MPI_ANY_TAG, MPI_COMM_WORLD, &recv_req[0]);
        }
        if (rank > 0) {
            MPI_Irecv(&temp[0], total_cols, MPI_DOUBLE, down_pr, MPI_ANY_TAG,
        ...
        // Wait for non-blocking receives to complete
        if (rank < numprocs - 1) {
...
void print_temp(void) {
    ...
    if (rank!=0) {
        ...
        MPI_Recv(&flg,1,MPI_CHAR,down_pr,MPI_ANY_TAG,MPI_COMM_WORLD,&status);
    }
    else {
    ...
    if (rank!=numprocs-1) {
        MPI_Send(&flg,1,MPI_CHAR,up_pr,123,MPI_COMM_WORLD);
...
unsigned long long int checksum(void) {
    ...
    if (rank!=0) {
        // if not rank 0 wait until adjacent left process has completed
        MPI_Recv(&sum,sizeof(double),MPI_CHAR,down_pr,MPI_ANY_TAG,
                MPI_COMM_WORLD,&status);
    }
    else {
    ...
    if (rank!=numprocs-1) {
        MPI_Send(&sum,sizeof(double),MPI_CHAR,up_pr,123,MPI_COMM_WORLD);
        if (rank==0) {
...
int main (int argc, char *argv[]){
    ...
    // is to process 
    int rows_per_processor = n / numprocs;
    int remainder_rows = n % numprocs;
    if (rank < remainder_rows) {
        active_rows_on_proc = rows_per_processor + 1;
    } else {
        active_rows_on_proc = rows_per_processor;
    }
